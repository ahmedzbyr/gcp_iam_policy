...
locals {
  ...
  bt_authoritative_access = local.authoritative ? { for perms in var.access : "${perms.permission}-${perms.product}-${perms.component}" => perms if perms.product == "BIGTABLE" } : {}
  ...
  # PUBSUB_TOPIC
  pbt_sa_permissions       = local.non_authoritative ? { for perms in local.acc_service_account : "${perms.permission}-${perms.product}-${perms.service_account}" => perms if perms.product == "PUBSUB_TOPIC" } : {}
  pbt_grps_permissions     = local.non_authoritative ? { for perms in local.acc_groups : "${perms.permission}-${perms.product}-${perms.group}" => perms if perms.product == "PUBSUB_TOPIC" } : {}
  pbt_authoritative_access = local.authoritative ? { for perms in var.access : "${perms.permission}-${perms.product}-${perms.component}" => perms if perms.product == "PUBSUB_TOPIC" } : {}
}
